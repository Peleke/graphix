...
export class GeneratedImageService {
  ...
  async create(data: {
    ...
    if (!data.steps || data.steps < 1 || data.steps > 150) {
      throw new Error("Steps must be between 1 and 150");
    }
    if (!data.cfg || data.cfg < 1 || data.cfg > 30) {
      throw new Error("CFG must be between 1 and 30");
    }
    if (!data.width || data.width < 256 || data.width > 4096) {
      throw new Error("Width must be between 256 and 4096");
    }
    if (!data.height || data.height < 256 || data.height > 4096) {
  ...
  async toggleFavorite(id: string): Promise<GeneratedImage> {
    ...
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }
  ...
  /**
   * Select an image (deselects others in same panel)
   */
  async select(id: string): Promise<GeneratedImage> {
    const existing = await this.getById(id);
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }

    // Deselect all others in the same panel
    await this.db
      .update(generatedImages)
      .set({ isSelected: false, updatedAt: new Date() })
      .where(eq(generatedImages.panelId, existing.panelId));
   ...
   * Deselect an image
   */
  async deselect(id: string): Promise<GeneratedImage> {
    const existing = await this.getById(id);
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }
   ...
   * Favorite an image
   */
  async favorite(id: string): Promise<GeneratedImage> {
    const existing = await this.getById(id);
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }
   ...
   * Unfavorite an image
   */
  async unfavorite(id: string): Promise<GeneratedImage> {
    const existing = await this.getById(id);
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }
   ...
   * Rate an image
   */
  async rate(id: string, rating: number): Promise<GeneratedImage> {
    const existing = await this.getById(id);
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }
  ...
  /**
   * Clear rating from an image
   */
  async clearRating(id: string): Promise<GeneratedImage> {
    const existing = await this.getById(id);
    if (!existing) {
      throw new Error(`Generated image not found: ${id}`);
    }
...
export function resetGeneratedImageService(): void {
  instance = null;
}
