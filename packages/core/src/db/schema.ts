...
// ============================================================================
// GENERATED TEXT (for storing AI-generated text content)
// ============================================================================

/**
 * Stores text generated by AI providers (Ollama, Claude, etc.).
 * Text can be associated with panels, pages, projects, or be standalone.
 * Supports independent viewing, editing, and regeneration.
 */
export const generatedTexts = sqliteTable(
  "generated_texts",
  {
    id: id(),
    // Optional associations (only one should be set, or none for standalone)
    panelId: text("panel_id").references(() => panels.id, { onDelete: "cascade" }),
    pageLayoutId: text("page_layout_id").references(() => pageLayouts.id, { onDelete: "cascade" }),
    projectId: text("project_id").references(() => projects.id, { onDelete: "cascade" }),
    // Text content
    text: text("text").notNull(),
    // Type of generated text
    textType: text("text_type").$type<GeneratedTextType>().notNull(),
    // Generation metadata
    provider: text("provider").notNull(), // "ollama", "claude", "openai"
    model: text("model").notNull(), // "llama3.2", "claude-sonnet-4-20250514", etc.
    // Token usage (nullable for providers that don't report)
    tokensUsed: integer("tokens_used"),
    inputTokens: integer("input_tokens"),
    outputTokens: integer("output_tokens"),
    // Generation context (what prompt was used)
    prompt: text("prompt"),
    ...
    // Status tracking
    status: text("status").$type<GeneratedTextStatus>().default("active").notNull(),
    // Edit tracking
    isEdited: integer("is_edited", { mode: "boolean" }).default(false).notNull(),
    editedAt: integer("edited_at", { mode: "timestamp" }),
    originalText: text("original_text"), // Store original before edits
    // Optional metadata (JSON for flexibility)
    metadata: text("metadata", { mode: "json" }).$type<GeneratedTextMetadata>(),
    ...timestamps,
  },
  (table) => [
    index("generated_texts_panel_idx").on(table.panelId),
    index("generated_texts_page_idx").on(table.pageLayoutId),
    index("generated_texts_project_idx").on(table.projectId),
    index("generated_texts_type_idx").on(table.textType),
    index("generated_texts_status_idx").on(table.status),
  ]
);

export type GeneratedTextType =
  | "panel_description"  // Panel visual description
...
export type GeneratedTextStatus =
  | "active"             // Currently in use
  | "archived"           // Kept but not in use
  | "superseded";        // Replaced by a newer version

export type GeneratedTextMetadata = {
  // For panel_description
  panelContext?: {
    setting?: string;
  ...
  // For caption suggestions
  captionSuggestions?: Array<{
    text: string;
    type: string;
    character?: string;
    confidence: number;
...
export type NewGeneratedText = typeof generatedTexts.$inferInsert;
...
export const projectsRelations = relations(projects, ({ many }) => ({
  ...
  generatedTexts: many(generatedTexts),
}));
...
export const generatedTextsRelations = relations(generatedTexts, ({ one }) => ({
  panel: one(panels, {
    fields: [generatedTexts.panelId],
    references: [panels.id],
  }),
  pageLayout: one(pageLayouts, {
    fields: [generatedTexts.pageLayoutId],
    references: [pageLayouts.id],
  }),
  project: one(projects, {
    fields: [generatedTexts.projectId],
    references: [projects.id],
  }),
}));
